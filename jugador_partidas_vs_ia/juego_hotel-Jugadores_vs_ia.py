# -*- coding: utf-8 -*-
"""juego_hotel-Jugadores_vs_IA.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1c-ozdgzMSYzZD2el46knjWx0SRIi-hMC
...
with open(f'hotel_compra.pkl', 'rb') as f:
    model_compra = pickle.load(f)
...
# ---------------------- OPERACIONES JUGADOR -------------------------- #
  ...
  def operar(self, num_partida):
    ...
    if len(propiedades_casilla) > 0: #Estoy en una casilla que tiene propiedades a los lados
      if juego.tablero.valor_casilla(self.casilla_actual) == "compra":
        if self.nombre == "IA":
          return self.comprar_propiedad(propiedades_casilla, True, num_partida)
        else:
          return self.comprar_propiedad(propiedades_casilla, False, num_partida)
  ...
  def comprar_propiedad(self, propiedades_casilla, IA, num_partida):
    ...
    if IA:
      if juego.tablero.valor_casilla(self.casilla_actual) == "compra":
        ...
        valor_casilla = 1
      input_variables = pd.DataFrame([[num_partida, self.casilla_actual, valor_casilla]], columns=['tirada', 'casilla', 'valor_casilla'], dtype=float)        

      lista_propiedades = ['no compra', 'Al Walid', 'Artika', 'Coconut', 'Dragon gate', 'Reef Resort', 'Reine', 'Up Town towers', 'Zebra Lodge']
      prop_IA = lista_propiedades[model_compra.predict(input_variables)[0]]
      if prop_IA == "no compra":
        comprar = False
      else:
        comprar = True
        if len(propiedades_casilla) > 1 :
          if propiedades_casilla[0].nombre == prop_IA:
            prop_compra = 0
          elif propiedades_casilla[1].nombre == prop_IA:
            prop_compra = 1
        else:
          if propiedades_casilla[0].nombre == prop_IA:
            prop_compra = 0
      # Si la casilla que ha predicho la IA no coincide con las propiedades de la casilla, se elige aleatoriamente la propieda a comprar
    ...
    if (propiedades_casilla[prop_compra] not in self.propiedades) and (propiedades_casilla[prop_compra].edificios_construidos == 0): # Compruebo que no es mia la propiedad o ya está comprada
      ...
      juego.registro.update(compra = "no")
      return "No compro esta propiedad es mía " + propiedades_casilla[prop_compra].nombre
...
for num_partida in range(100):
  #Inicialización valores de la partida
  jugadores = [Jugador("IA"), Jugador("Jose"), Jugador("María"), Jugador("Rosa")]
  ...
  while len(juego.jugadores) > 1:
    for j in juego.jugadores:
      ...
      if j.dinero >= 0 and len(j.propiedades) >=0:
        ...
        j.mover(tirada)
        print(str(num_partida) + "-" + str(num_tirada) + "-> " + j.nombre + ":\tdados\t", tirada, " casilla ", j.casilla_actual, " (", juego.tablero.valor_casilla(j.casilla_actual), ")\toperacion ", j.operar(num_tirada), "\tdinero ", j.dinero)
...
df.to_csv("Cien_Partidas_Hotel-Jugadores_vs_IA.csv", index=False)